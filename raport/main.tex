...
Această secțiune demonstrează că, după aplicarea fix-urilor pe branch-ul \texttt{secured},
atacurile prezentate anterior nu mai funcționează.
...
Validarea input-ului respinge parolele triviale, iar baza de date nu mai conține
parole în plaintext --- atacul de credential stuffing este blocat din start.
...
\begin{figure}[H]
  ...
  \caption{Rate limiter activ: cereri excesive returnează 429 Too Many Requests}
\end{figure}

Scriptul de brute force este oprit de rate limiter după 10 cereri/minut per IP.
Contul se blochează automat după 5 încercări eșuate consecutive (423 Locked).
...
\begin{figure}[H]
  ...
  \caption{Mesaj uniform "Invalid credentials" indiferent de cauza erorii}
\end{figure}

Atât pentru email inexistent cât și pentru parolă greșită, răspunsul este identic ---
enumerarea utilizatorilor nu mai este posibilă.
...
După logout, token-ul este adăugat în blacklist. Orice cerere ulterioară cu același
token este respinsă cu \texttt{401 Token has been invalidated}, chiar dacă JWT-ul
nu a expirat.
...
Token-ul de resetare este acum generat criptografic (\texttt{crypto/rand}, 32 bytes)
și este șters din baza de date imediat după utilizare. O a doua cerere cu același
token returnează \texttt{401 Invalid token}.
...
Sistemul înregistrează automat fiecare acțiune relevantă în tabelul \texttt{audit\_logs}.
Endpoint-ul \texttt{GET /api/audit} (accesibil doar managerilor) oferă vizibilitate
completă asupra activității din aplicație.

\begin{figure}[H]
  \centering
  \includegraphics[width=\textwidth]{audit_logs.png}
  \caption{Jurnal de audit vizualizat prin \texttt{/api/audit} — acțiuni REGISTER, LOGIN, FAILED\_LOGIN, LOGOUT}
\end{figure}
...
\begin{itemize}
    ...
    \item \textbf{user\_id} --- ID-ul utilizatorului care a efectuat acțiunea
    \item \textbf{ip\_address} --- adresa IP a clientului
    \item \textbf{timestamp} --- momentul exact al evenimentului
    \item \textbf{resource} / \textbf{resource\_id} --- resursa afectată (ticket, auth)
\end{itemize}

În captura de mai sus se observă clar urmele atacului de brute force
(multiple \texttt{FAILED\_LOGIN} consecutive pentru același \texttt{user\_id}),
urmate de înregistrări în masă (\texttt{REGISTER}) din scriptul de credential stuffing.
Audit trail-ul permite identificarea rapidă a comportamentului malițios.
...
\begin{enumerate}
    ...
    de autentificare este vulnerabil la brute force indiferent de complexitatea parolei.

    \item \textbf{Mesajele de eroare dezvăluie informații.} Răspunsuri diferite pentru
    ``email inexistent'' vs. ``parolă greșită'' permit enumerarea utilizatorilor.
    Mesajul trebuie să fie uniform.

    \item \textbf{Cookie-urile de sesiune trebuie protejate.} \texttt{HttpOnly} previne
    ...
    real necesită o blacklist server-side. Token-urile de resetare parolă trebuie șterse
    imediat după utilizare.

    \item \textbf{Audit logging este critic.} Fără jurnal de acțiuni, un atac poate trece
    neobservat. Logarea \texttt{FAILED\_LOGIN} permite detectarea brute force în timp real.

    \item \textbf{Separarea versiunilor vulnerable/secured} facilitează înțelegerea
    impactului fiecărei vulnerabilități și validarea clară a fix-urilor prin re-testare.
\end{enumerate}
